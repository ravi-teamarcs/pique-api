...
export class MediaController {
    ...
    async uploadMedia(
        ...
        if (!files.images && !files.videos && !files.headshot) {
            throw new BadRequestException('At least one media file (images, videos, or headshot) is required.');
        }
        ...
        if (files.images) {
            for (const file of files.images) {
                ...
                uploadedFiles.push({ url: filePath, name: file.originalname, type: 'image' });
            }
        }
        ...
        if (files.videos) {
            for (const file of files.videos) {
                const filePath = await uploadFile(file);
                uploadedFiles.push({ url: filePath, name: file.originalname, type: 'video' });
            }
        }
        ...
        if (files.headshot) {
            for (const file of files.headshot) {
                const filePath = await uploadFile(file);
                uploadedFiles.push({ url: filePath, name: file.originalname, type: 'headshot' });
            }
        }
    ...
    async updateMedia(
        ...
        @UploadedFiles() files: { images?: Express.Multer.File[], videos?: Express.Multer.File[], headshot?: Express.Multer.File[] },
    ) {
        const { userId, RefId } = body;
        console.log(userId, files);

        // Validate only one type of media is uploaded
        const fileTypes = Object.keys(files).filter((key) => files[key]?.length > 0);
