...
export class EntertainerService {
    ...
    async getAllEntertainers({
        ...
        const [records, total] = await this.entertainerRepository.findAndCount({
            where: {
                ...
                status: In(['active', 'available']), // Only fetch records where status is 'active'
            },
    ...
    async update(updateEntertainerDto: UpdateEntertainerDto): Promise<any> {
        ...
        await this.entertainerRepository.update(existingEntertainer.id, fieldsToUpdate);


        return this.entertainerRepository.findOne({ where: { id: existingEntertainer.id } });
    }
    ...
    async updateCategory(updateCategoryDto: UpdateCategoryDto): Promise<Categories> {
    ...
    async updateStatus(updateStatusDto: UpdateStatusDto): Promise<string> {
        ...
        const userToUpdate = await this.entertainerRepository.findOne({ where: { id } });
        ...
        // Perform the update
        try {
            const result = await this.entertainerRepository.update(id, { status });
    ...
    //     return this.entertainerRepository
    //         .createQueryBuilder('entertainer')
    //         .where('LOWER(entertainer.name) LIKE :query', {
