...
import { Invoice, InvoiceStatus, UserType } from 'src/modules/admin/invoice/Entity/invoices.entity';
...
export class GenerateInvoiceService {
    ...
    async createInvoice(event: Event) {
        ...
        const issueDate = isSameMonth ? today : new Date(eventDate.getFullYear(), eventDate.getMonth(), 1);
        ...
        //     const entertainer = await this.entertainerRepo.findOne({ where: { user: booking.eId } });
        //     const taxRate = 10.0; // Example tax rate (10%)
        ...
        //     const lastInvoiceNumber = lastInvoice ? parseInt(lastInvoice.invoice_number.split('-')[1]) : 1000;
        ...
        //         payment_method: '',
        //         payment_date: null, // Ensure it's null if no date is provided
        //     };

        //     console.log('New Invoice:', newinvoice);

        //     const newInvoice = this.invoiceRepo.create(newinvoice);
        ...
        for (const booking of bookings) {  // Process sequentially instead of in parallel
            const entertainer = await this.entertainerRepo.findOne({ where: { user: booking.eId } });
            const taxRate = 10.0;
